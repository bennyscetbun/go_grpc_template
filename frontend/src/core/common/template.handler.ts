...
export class TemplateHandler {
    ...
    findPropBindings() {
        ...
        this.bindToPropertiesBothWay('bind-bothway-innerHTML');
        this.bindToPropertiesBothWay('bind-bothway-innerText');
        this.bindToPropertiesBothWay('bind-bothway-value');
    }
    findEvents() {
        //bind to click event
    ...
    bindToPropertiesBothWay(attr: string) {
        let props = document.querySelectorAll("[" + attr + "]");
        let actualProp = attr.replace('bind-bothway-', '');
        ...
        props.forEach((elm) => {
            ...
            if (Reflect.has(this.global.currentRoute.props, attribVal)) {
                ...
                elm.addEventListener('input', (e: any) => {
                        this.global.currentRoute.props[attribVal] = e.target.value;
                });
            } else {
