...
function generate_proto() {
    ...
    (cd .. && mkdir -p frontend/src/generated/rpc/ backend/generated/rpc/ && docker run --user $(id -u):$(id -g) --rm -v./common/proto:/proto -w/proto -v./backend/generated/rpc:/gogen -v./frontend/src/generated/rpc:/jsproto xxx_your_app_xxx/protoc -I=. $list_proto_files   --js_out=import_style=commonjs:/jsproto   --grpc-web_out=import_style=typescript,mode=grpcweb:/jsproto --go_out=/gogen --go_opt=paths=source_relative     --go-grpc_out=/gogen --go-grpc_opt=paths=source_relative)
}
